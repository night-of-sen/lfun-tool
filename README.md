...
| 等级 | 判定依据 | 数量 | 含义 |
|---|---|---|---|
| 🟢 `ready` | 有官网在线版 | **79** | 打开就能用，不用装也不用部署 |
| 🟡 `cloud` | 厂商提供官方云版 | **49** | 注册后即可使用，不用自己部署；免费额度或试用期以厂商为准 |
| 🔵 `onecmd` | 自托管 + README 里有可执行命令 | **27** | 需要服务器，但能复制粘贴一条命令跑起来 |
| 🟠 `setup` | 自托管但 README 里没有现成命令 | **89** | 需要服务器 + 配数据库/环境变量/域名，得看官方文档 |
| ⚪ `install` | 桌面应用 / 命令行 / 移动端 | **236** | 下载安装到本机，不需要服务器 |
| 📦 `library` | 开发库 | **64** | 不是独立应用，要写代码调用 |
| 📚 `resource` | 清单 / 课程 / 教程 | **26** | 没有东西要装，打开仓库读 |
...
- **按钮按用途自动变化**：
  ...
  | 使用方式 | 主按钮 | 说明 |
  |---|---|---|
  | 有云版 | 云版 → 厂商托管地址 | **优先级最高**：厂商有托管版就直接给最短路径 |
