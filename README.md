...
- 之所以改：分类涨到 49 个之后，横向筛选条要左右滑动才能看全，很难用
...
- 首页有 535 条指向工具页的内链，工具页有「同类工具」反向内链，形成内链网。
...
- **英文页标签走映射**：`i18n.json` 里的 `tagTranslations`（315 条，覆盖率 100%）；
  搜索索引同时包含原文和译文标签，中英文都能搜到。
- `sitemap.xml` 自动生成，含 `xhtml:link` 多语言标注，1436 个 URL 条目。
...
所以能直接访问的文件必须在根目录。
    ...
    ├─ tool/<id>/index.html        ← 535 个中文工具页（生成）
    ├─ category/<key>/index.html   ← 49 个中文分类页（生成）
    ├─ categories/index.html       ← 分类总览（生成）
    ├─ en/tool/<id>/index.html     ← 535 个英文工具页（生成）
    ├─ en/category/<key>/index.html← 49 个英文分类页（生成）
    ├─ en/categories/index.html    ← 英文分类总览（生成）
    ├─ compare/<a>-vs-<b>/index.html ← 109 组对比页（生成）
    ├─ online/                     ← 站内在线工具（生成）
    │   ├─ index.html              ← 工具索引
...
- 未认证时 GitHub API 限额为 **core 60 次/小时**，而项目有 535 个——
...
- 摘要只接受**以项目名开头**的段落（"X is a …" 这种）。更宽松的规则会放进
  "Optional: set APIURL…" 这类配置说明，所以宁可少也要准。当前 361/535 有摘要。
- 工具页的视觉图：**有官网地址就用 WordPress mShots 截真实网页**，没有则回退到 GitHub
  自动生成的仓库卡片（1200×600，零维护）。
- **搜索别名**：`scripts/aliases.json` 给 464 个工具配了 768 条中文俗称和简称，
...
- **首页 HTML 约 906 KB**：535 张卡片全在 HTML 里是为了 SEO。开 gzip 后约 135 KB。
